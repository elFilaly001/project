...
export default function FollowersByCityCard() {
  ...
  const topCity = cityData.reduce((a, b) => (b.value > a.value ? b : a), cityData[0]);
  const interpretationSentences = [
    `${topCity.label} is the largest audience city (${topCity.value.toFixed(2)}%).`,
    `Content tailored to this city may increase engagement.`,
    `Consider strategies to engage audiences in other cities for broader reach.`
  ];
