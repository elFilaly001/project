...
export default function CompetitiveTable(): React.ReactElement {
  ...
  const menu = open && menuPos ? (
    createPortal(
      <ul ref={menuRef} className="bg-white border rounded shadow-lg p-2 text-sm" style={{ position: 'absolute', top: menuPos.top + 'px', left: menuPos.left + 'px', zIndex: 9999, minWidth: 160 }} role="listbox">
        {options.map((opt) => (
          <li key={opt} onClick={() => { setSelectedPlatform(opt); setOpen(false); setMenuPos(null) }} className="px-3 py-1 hover:bg-gray-100 rounded cursor-pointer" role="option" aria-selected={selectedPlatform === opt}>
            {opt}
          </li>
        ))}
      </ul>,
      ...
      <div className="flex items-center justify-between gap-4 mb-3">
        <div className="flex items-center gap-2">
          <input
            ...
            placeholder="Search competitors (handle, followers, activity...)"
        ...
        <div className="text-sm text-gray-500">Platform: <strong>{selectedPlatform}</strong></div>
      </div>
