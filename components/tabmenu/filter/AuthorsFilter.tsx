...
export default function AuthorsFilter() {
    ...
    const label = value === 'all' ? 'All' : (value === 'general_public' ? 'General public' : value);
        ...
        <div className="rounded-lg shadow-sm border bg-white dark:bg-slate-800 ring-1 ring-slate-100 dark:ring-slate-700">
            <Popover open={open} onOpenChange={setOpen}>
                <PopoverTrigger asChild>
                    <button title="Filter by author — General public, influencers, media, competitors" ref={triggerRef} className="w-full px-4 py-3 flex items-center gap-3 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors rounded-lg group">
                        ...
                        <div className="flex-1 text-left min-w-0">
                            <div className="text-sm font-medium text-slate-900 dark:text-slate-100">By author</div>
                            <div className="text-xs text-slate-600 dark:text-slate-400 truncate">{label}</div>
                        </div>
                        <span className="ml-2 inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium bg-slate-100 text-slate-700 dark:bg-slate-700 dark:text-slate-300">Author</span>
                ...
                <PopoverContent align="start" className="w-auto" style={minWidthPx ? { minWidth: `${minWidthPx}px` } : undefined}>
                    <div className="flex flex-col gap-3">
                        ...
                        <p className="text-xs text-slate-500 dark:text-slate-400">General public, influencers, media, competitors</p>

                        <div className="flex flex-col divide-y rounded-md border bg-white dark:bg-slate-800">
                            {[
                                { v: 'all', label: 'All' },
                                { v: 'general_public', label: 'General public' },
                                { v: 'influencers', label: 'Influencers' },
                                { v: 'media', label: 'Media' },
                                { v: 'competitors', label: 'Competitors' },
                            ].map((opt) => (
                                <button
                                    key={opt.v}
